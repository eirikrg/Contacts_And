...
public class UsersMapper implements Mapper<UsersApiResponse, List<User>> {
  ...
  public List<User> mapToDomain(UsersApiResponse usersApiResponse) {
    return usersApiResponse.getResults().stream()
        .map(
            apiUserModel ->
                new User(
  ...
  private Name mapApiName(NameModel apiName) {
    return new Name(apiName.getTitle(), apiName.getFirst(), apiName.getLast());
  }
  ...
  private Street mapApiStreet(StreetModel apiStreet) {
    return new Street(apiStreet.getNumber(), apiStreet.getName());
  }

  private Coordinates mapApiCoordinates(CoordinatesModel apiCoordinates) {
    return new Coordinates(apiCoordinates.getLatitude(), apiCoordinates.getLongitude());
  }

  private Timezone mapApiTimezone(TimezoneModel apiTimezone) {
    return new Timezone(apiTimezone.getDescription(), apiTimezone.getDescription());
  }
  ...
  private Dob mapApiDob(DobModel apiDob) {
    return new Dob(apiDob.getDate(), apiDob.getAge());
  }

  private Registered mapApiRegistered(RegisteredModel apiRegistered) {
    return new Registered(apiRegistered.getDate(), apiRegistered.getAge());
  }

  private Id mapApiId(IdModel apiId) {
    return new Id(apiId.getName(), apiId.getValue());
  }

  private Picture mapApiPicture(PictureModel apiPicture) {
    return new Picture(apiPicture.getLarge(), apiPicture.getMedium(), apiPicture.getThumbnail());
  }
}
