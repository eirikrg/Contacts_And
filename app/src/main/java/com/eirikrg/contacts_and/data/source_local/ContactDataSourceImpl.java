...
public class ContactDataSourceImpl implements ContactDataSource {
    ...
    private static void generateNameOperation(User contact, ArrayList<ContentProviderOperation> operations) {
        ContentProviderOperation.Builder builder;
        builder = ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI);
        builder.withValueBackReference(ContactsContract.Data.RAW_CONTACT_ID, 0);
        builder.withValue(ContactsContract.Data.MIMETYPE, ContactsContract.CommonDataKinds.StructuredName.CONTENT_ITEM_TYPE);
        builder.withValue(ContactsContract.CommonDataKinds.StructuredName.DISPLAY_NAME, contact.getName().getFullName());
        operations.add(builder.build());
    }

    private static void generateCellphoneOperatrion(User contact, ArrayList<ContentProviderOperation> operations) {
        ContentProviderOperation.Builder builder;
        builder = ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI);
        builder.withValueBackReference(ContactsContract.Data.RAW_CONTACT_ID, 0);
